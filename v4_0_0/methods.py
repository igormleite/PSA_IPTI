...
"""
Este método gera uma matriz de similaridade para cada agrupamento dado como entrada de acordo com uma métrica definida (por exemplo uma norma envolvendo os macronutrientes e valor energético)
"""

def similarityMatrix(config, grouping, foods):
    result = []
    for group in grouping:
        if len(group['items']) != 1:
            ...
            for itemsGroup in group["items"]:
                cand = utils.getFood(itemsGroup["code"], foods, grouping)
                if cand == None:
                    raise Exception("O alimento <" + str(itemsGroup["code"]) + "> não foi encontrado")
                else:
                    candidatesMatrix.append([cand["calorie"],cand["protein"], cand["lipid"], cand["carbo"]])
                    candidatesId.append({'code':cand['code'],
                                         'name':cand['name']})
            ...
            # Caculos para o total de opções exibidas
            for i in range(len(candidatesId)):
                ...
                # Função "computeLights" requer o recebimento de uma lista como segundo parametro
                colors = utils.computeColors(config, similarityMatrix[i].tolist())
                suggestions = []
                for ii in auxObj:
                    # Agrupa as listas de sugestões para os pratos 
                    suggestions.append({'name':candidatesId[ii[0]]['name'],
                                        'code':candidatesId[ii[0]]['code'],
                                        'score':ii[1],
                                        'semaphore':colors[ii[0]]})
                #Agrupa as sugestões como função da Escola, tipo de refeição, prato e sugestões
                result.append({'codeReference': candidatesId[i]['code'],
                               'nameReference': candidatesId[i]['name'],
                               "group": group['name'],
                               "itemHint": suggestions})
        else: # Se o item não possuir um substituto no agrupamento
            # Identifica o item e retorna sugestão vazia
            candidatesId = utils.getFood(group["items"][0]['code'], foods, grouping)
            result.append({'codeReference': candidatesId['code'],
                           'nameReference': candidatesId['name'],
                           "group": group['name'],
                           "itemHint": []})
...
"""
Método utilizado para gerar substituições dado um cardápio de entrada através da ordenação de sugestões para substituição de acordo com um critério definido.

Os alimentos sugeridos levam em consideração a disponibilidade de estoque e alimentos pertencentes a um mesmo agrupamento.
"""

def findSuggest(config, menu, grouping, inventory, foods):
